...
1. Altere o código da função `bubble_sort` para interromper as passagens quando
   não houver mais trocas.
1. Compare o tempo de execução das duas versões da função `bubble_sort` para
...
1. Altere o código da função `shaker_sort` para interromper as passagens quando
   não houver mais trocas.
1. Compare o tempo de execução das duas versões da função `shaker_sort` para
   vetores muito grandes.
...
```cpp
template <typename ForwardIterator>
void merge(ForwardIterator begin, ForwardIterator mid, ForwardIterator end)
{
  ...
  std::move(it1, mid, std::back_inserter(tmp));
  std::move(it2, end, std::back_inserter(tmp));
